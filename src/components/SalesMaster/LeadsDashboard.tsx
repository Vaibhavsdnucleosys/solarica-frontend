...
const LeadStatsCard = ({
  ...
  >
    <div
      className={`absolute -inset-1 bg-gradient-to-br ${color} opacity-0 group-hover:opacity-10 transition-opacity duration-500 rounded-xl blur-lg`}
    />
    ...
    <div className="relative bg-white/70 backdrop-blur-xl p-4 rounded-xl border border-white shadow-[0_8px_32px_rgba(0,0,0,0.03)] group-hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] transition-all duration-500">
      <div className="flex justify-between items-start mb-3">
        <div
          className={`p-2 rounded-lg bg-white shadow-sm border border-slate-100 ${color.replace("from-", "text-").split(" ")[0]}`}
        >
          <Icon size={18} />
        </div>
        {trend && (
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${color} opacity-10 group-hover:opacity-100 transition-opacity`}
      />
    </div>
  </div>
);
...
const LeadsDashboard: React.FC<LeadsDashboardProps> = ({ user }) => {
  ...
  const userLeads = useMemo(() => {
    ...
    return leads.filter(
      (lead: any) =>
        ...
        lead.createdBy?.id === currentUser.id,
    );
  }, [leads, currentUser?.id, isAdmin]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  const filteredLeads = useMemo(() => {
    ...
    let result = userLeads.filter((lead) => {
      const matchesDate = new Date(lead.createdAt) >= cutoff;
      const matchesSearch =
        lead.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        lead.company?.toLowerCase().includes(searchQuery.toLowerCase()) ||
 ...
 const stats = useMemo(() => {
   ...
   const lost = filteredLeads.filter(
      l => l.status === 'lost'
   ).length;
                    ...
                    {[1, 2, 3, 4].map((range) => (
                      ...
                      >
                        ...
                        <ChevronRight
                          size={14}
                          className={
                            filterRange === range ? "opacity-100" : "opacity-0"
                          }
                        />
                      </button>
                    ))}
