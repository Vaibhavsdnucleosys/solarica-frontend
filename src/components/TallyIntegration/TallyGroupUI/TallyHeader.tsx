...
const TallyHeader = (props: {
    ...
    return (
        <>
            <div className="relative z-[9999] flex flex-col w-full bg-[#1a202c] shadow-lg h-[52px] select-none text-white overflow-visible font-sans backdrop-blur-md bg-opacity-95">
                ...
                <div className="flex items-center justify-between h-[28px] mt-[1px] px-2 relative w-full">
                    ...
                    <div className="absolute left-1/2 transform -translate-x-1/2 w-[45%] max-w-[650px] z-10">
                        <div className="flex items-center bg-gray-800/80 rounded-[4px] h-[24px] px-2 shadow-inner border border-gray-600 focus-within:border-[#feba35] transition-all">
                            ...
                            <input
                                ...
                                className="bg-transparent border-none outline-none text-[12px] w-full text-gray-100 placeholder-gray-500 font-semibold h-full"
                            />
                        </div>
                    </div>

                    {/* Right Side: Sales Data Toggle + System Icons */}
                    <div className="flex items-center gap-3 ml-auto">
                        {/* Sales Data Toggle Button */}
                        <button
                            onClick={props.onToggleSalesData}
                            className={`px-4 py-1 rounded-md text-[11px] font-bold transition-all duration-200 border ${props.showSalesData
                                ? 'bg-[#feba35] text-black border-[#feba35] shadow-lg'
                                : 'bg-gray-700/80 text-gray-200 border-gray-600 hover:bg-gray-600 hover:border-gray-500'
                                }`}
                            title={props.showSalesData ? "Switch to Tally View" : "View Sales Data"}
                        >
                            <span className="flex items-center gap-1.5">
                                <span>{props.showSalesData ? '📊' : '📈'}</span>
                                <span>{props.showSalesData ? 'Sales Data' : 'Sales Info'}</span>
                            </span>
                        </button>

                        {/* System Icons */}
                        <div className="flex items-center text-gray-400 gap-4 scale-[0.85] origin-right ml-2">
                            <svg className="w-5 h-5 cursor-center hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 12H4"></path></svg>
                            <svg className="w-5 h-5 cursor-center hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 3H5a2 2 0 00-2 2v3m18 0V5a2 2 0 00-2-2h-3m0 18h3a2 2 0 002-2v-3M3 16v3a2 2 0 002 2h3"></path></svg>
                            <svg className="w-5 h-5 cursor-pointer hover:text-[#feba35]" fill="none" stroke="currentColor" viewBox="0 0 24 24" onClick={() => props.onAction?.('quit')}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </div>
                    </div>
                </div>

                {/* Bottom Row: Navigation Menus & Go To */}
                <div className="flex items-center h-[24px] pb-1 w-full relative mt-auto px-1 bg-[#1a202c]">
                    <div className="flex items-center gap-1">

                        {/* Go To Button (Centered) */}
