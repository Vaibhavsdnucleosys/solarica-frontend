...
interface Props {
    companyId: string;
    onClose: () => void;
    onSelect: (groupName: string, mode: 'Create' | 'Alter') => void;
}

const flattenGroups = (items: TallyItem[]): string[] => {
    let flat: string[] = [];
    items.forEach(item => {
        flat.push(item.name);
        if (item.children) {
            flat = flat.concat(flattenGroups(item.children));
        }
    });
    return flat.sort();
};

import { getAccountGroups } from '../../../services/accountingService';

const TallyGroupSelectModal: React.FC<Props> = ({ companyId, onClose, onSelect }) => {
    ...
    useEffect(() => {
        const fetchGroups = async () => {
            try {
                ...
                // Extract unique names from hierarchical/flattened data
                const names = new Set<string>();
                const processGroups = (nodes: any[]) => {
                    nodes.forEach(node => {
                        names.add(node.name);
                        if (node.children) processGroups(node.children);
                    });
                };
                processGroups(groups || []);
                setUniqueGroups(Array.from(names).sort());
            } catch (error) {
    ...
    const filteredItems = uniqueGroups.filter(name =>
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'Enter') {
                ...
                if (filteredItems.length > 0) {
                    onSelect(filteredItems[selectedIndex], 'Alter');
                }
            }
        };
    ...
    return (
        <div className="absolute inset-0 z-[1000] flex items-start justify-center pt-[55px] pointer-events-none">
            <div className="w-[400px] shadow-[0_15px_60px_rgba(0,0,0,0.4)] flex flex-col pointer-events-auto bg-white border border-[#2a5585]">
                {/* Parent Title - Background Context */}
                ...
                <div className="bg-[#2a5585] text-white px-2 py-0.5 flex justify-between items-start text-[12px] font-bold min-h-[36px]">
                    <span className="pl-1 pt-0.5 font-bold">List of Groups</span>
                ...
                {/* List Content */}
                <div className="bg-[#e8f6fa] min-h-[300px] max-h-[500px] overflow-y-auto custom-scrollbar">
                    {loading ? (
                        <div className="px-3 py-2 text-[13px] text-gray-500 italic text-center">Loading groups...</div>
                        ...
                        <div className="px-3 py-2 text-[13px] text-gray-500 italic text-center">No matches found</div>
                    )}
                </div>

                {/* Bottom Status Bar */}
                <div className="bg-[#e8f6fa] px-2 py-0.5 flex justify-between items-center border-t border-[#9bc9d9]">
                    <span className="text-[11px] text-gray-500">{filteredItems.length} groups</span>
...
export default TallyGroupSelectModal;
