...
import TallyPayrollVoucherCreation from './TallyPayrollVoucherCreation';
import TallyHeader from '../TallyGroupUI/TallyHeader';
import { TallySidebar, SidebarButton } from '../TallyGroupUI/TallySidebar';

interface TallyPayrollVoucherTypeListProps {
    onClose?: () => void;
    companyId: string;
}

const TallyPayrollVoucherTypeList: React.FC<TallyPayrollVoucherTypeListProps> = ({ onClose, companyId }) => {
    const [showCreationModal, setShowCreationModal] = useState(false);
    const [selectedIndex, setSelectedIndex] = useState(0);

    const voucherTypes = ['Attendance', 'Payroll'];
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'ArrowUp') {
                e.preventDefault();
                setSelectedIndex(prev => (prev > 0 ? prev - 1 : voucherTypes.length - 1));
            } else if (e.key === 'ArrowDown') {
                e.preventDefault();
                setSelectedIndex(prev => (prev < voucherTypes.length - 1 ? prev + 1 : 0));
            }
    ...
    }, [onClose, showCreationModal, voucherTypes.length]);
    ...
    return (
        <div className="w-full h-full flex flex-row bg-[#e8f6fa] font-sans overflow-hidden">
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="bg-white px-4 py-1 border-b border-gray-300">
                    <div className="flex justify-between items-center">
                        <span className="font-bold text-[13px]">List of Payroll Voucher Types</span>
                ...
                <div className="flex-1 overflow-y-auto bg-white custom-scrollbar relative">
                    ...
                    {voucherTypes.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-gray-400 mt-20">
                            <span className="font-bold text-[15px]">No voucher types found</span>
                        </div>
                    ) : (
                        <div className="flex flex-col">
                            {voucherTypes.map((vch, index) => (
                                <div
                                    key={index}
                ...
                <div className="bg-[#8ec2eb] border-t border-[#5ea4d6] px-4 py-1 flex justify-between">
                    <span className="text-[11px] font-bold text-black uppercase tracking-wider">{voucherTypes.length} Voucher Type(s)</span>
                    <span className="text-[11px] font-bold text-black uppercase tracking-wider">{selectedIndex + 1} of {voucherTypes.length}</span>
                </div>
            ...
            <TallySidebar>
                ...
                <SidebarButton keyName="SPACER" label="SPACER" />
                <SidebarButton keyName="B" label="Basis of Values" />
                <SidebarButton keyName="H" label="Change View" />
                <SidebarButton keyName="J" label="Exception Reports" />
                <SidebarButton keyName="L" label="Save View" />
                <SidebarButton keyName="SPACER" label="SPACER" />
                <SidebarButton keyName="E" label="Apply Filter" />
                <SidebarButton keyName="F" label="Filter Details" />
                <SidebarButton keyName="SPACER" label="SPACER" />
                <SidebarButton keyName="H" label="Multi-Masters" />
            </TallySidebar>
...
export default TallyPayrollVoucherTypeList;
