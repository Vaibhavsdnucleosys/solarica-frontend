...
interface Props {
    companyId: string;
    onClose: () => void;
    onSelect: (employeeName: string, mode: 'Create' | 'Alter') => void;
}

import { getPayrollEmployees } from '../../../services/accountingService';

const TallyEmployeeSelectModal: React.FC<Props> = ({ companyId, onClose, onSelect }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setAllItems((employees || []).map((e: any) => ({
                    label: e.name,
                    type: 'item',
                    parent: e.employeeGroup?.name || 'Primary'
                })));
            } catch (error) {
        ...
        <div className="absolute inset-0 z-[1000] flex items-start justify-center pt-[55px] pointer-events-none">
            <div className="w-[400px] shadow-[0_15px_60px_rgba(0,0,0,0.4)] flex flex-col pointer-events-auto bg-white border border-[#2a5585]">
                ...
                <div className="bg-[#2a5585] text-white px-2 py-0.5 flex justify-between items-start text-[12px] font-bold min-h-[36px]">
                    <span className="pl-1 pt-0.5">List of Employees</span>
                ...
                <div className="bg-[#e8f6fa] min-h-[300px] max-h-[500px] overflow-y-auto">
                    {loading ? (
                        <div className="px-3 py-2 text-[13px] text-gray-500 italic">Loading employees...</div>
...
export default TallyEmployeeSelectModal;
