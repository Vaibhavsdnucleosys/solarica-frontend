...
const TallyEmployeeGroupSelectModal: React.FC<Props> = ({ companyId, onClose, onSelect }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setAllItems((groups || []).map((g: any) => ({
                    label: g.name,
                    type: 'item',
                    parent: g.parent?.name || 'Primary'
                })));
            } catch (error) {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'Enter') {
                ...
                if (filteredItems.length > 0) {
                    ...
                    // If empty search and enter, maybe do nothing or select first if available
                }
            }
        };
        window.addEventListener('keydown', handleKeyDown);
        return () => window.removeEventListener('keydown', handleKeyDown);
    }, [selectedIndex, onClose, onSelect, filteredItems, searchTerm]);
    ...
    return (
        <div className="absolute inset-0 z-[1000] flex items-start justify-center pt-[55px] pointer-events-none">
            <div className="w-[400px] shadow-[0_15px_60px_rgba(0,0,0,0.4)] flex flex-col pointer-events-auto bg-white border border-[#2a5585]">
                {/* Parent Title (Mocking the background context) */}
                ...
                <div className="bg-[#2a5585] text-white px-2 py-0.5 flex justify-between items-start text-[12px] font-bold min-h-[36px]">
                    <span className="pl-1 pt-0.5">List of Employee Groups</span>
                ...
                <div className="bg-[#e8f6fa] min-h-[300px] max-h-[500px] overflow-y-auto">
                    {loading ? (
                        <div className="px-3 py-2 text-[13px] text-gray-500 italic">Loading groups...</div>
                        ...
                        filteredItems.map((item, idx) => (
                            ...
                            >
                                ...
                                <span className="italic font-normal text-[11px] pr-1">♦ {item.parent}</span>
                            </div>
                        ))
                    ) : (
...
export default TallyEmployeeGroupSelectModal;
