...
const TallyPayrollUnitSelectModal: React.FC<Props> = ({ companyId, onClose, onSelect }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedIndex, setSelectedIndex] = useState(0);
    const searchInputRef = useRef<HTMLInputElement>(null);

    const allItems = [
        { label: 'Hrs', type: 'item', formalName: 'Hours' },
        { label: 'Min', type: 'item', formalName: 'Minutes' },
        { label: 'Days', type: 'item', formalName: 'Days' },
        { label: 'Nos', type: 'item', formalName: 'Numbers' }
    ];

    const filteredItems = allItems.filter(item =>
        item.label.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.formalName.toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        <div className="absolute inset-0 z-[1000] flex items-start justify-center pt-[55px] pointer-events-none">
            <div className="w-[400px] shadow-[0_15px_60px_rgba(0,0,0,0.4)] flex flex-col pointer-events-auto bg-white border border-[#2a5585]">
                ...
                <div className="bg-[#2a5585] text-white px-2 py-0.5 flex justify-between items-start text-[12px] font-bold min-h-[36px]">
                    <span className="pl-1 pt-0.5">List of Units</span>
                    ...
                    {filteredItems.length > 0 ? (
                        filteredItems.map((item, idx) => (
                            ...
                            >
                                ...
                                <span className="italic font-normal text-[11px] pr-1">{item.formalName}</span>
                            </div>
                        ))
                    ) : (
...
export default TallyPayrollUnitSelectModal;
