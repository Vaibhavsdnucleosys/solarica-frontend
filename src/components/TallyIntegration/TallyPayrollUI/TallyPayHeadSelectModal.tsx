...
interface Props {
    companyId: string;
    onClose: () => void;
    onSelect: (name: string, mode: 'Create' | 'Alter') => void;
}

import { getPayHeads } from '../../../services/accountingService';

const TallyPayHeadSelectModal: React.FC<Props> = ({ companyId, onClose, onSelect }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedIndex, setSelectedIndex] = useState(0);
    const searchInputRef = useRef<HTMLInputElement>(null);

    // Mock data mimicking Tally Prime
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const payHeads = await getPayHeads(companyId);
                setAllItems((payHeads || []).map((p: any) => ({
                    label: p.name,
                    type: 'item',
                    category: p.payHeadType.replace(/_/g, ' ')
                })));
            } catch (error) {
    ...
    return (
        <div className="absolute inset-0 z-[1000] flex items-start justify-center pt-[55px] pointer-events-none">
            <div className="w-[450px] shadow-[0_15px_60px_rgba(0,0,0,0.4)] flex flex-col pointer-events-auto bg-white border border-[#2a5585]">
                {/* Parent Title */}
                ...
                <div className="bg-[#2a5585] text-white px-2 py-0.5 flex justify-between items-start text-[12px] font-bold min-h-[36px]">
                    <span className="pl-1 pt-0.5">List of Pay Heads</span>
                ...
                <div className="bg-[#e8f6fa] min-h-[300px] max-h-[500px] overflow-y-auto">
                    {loading ? (
                        <div className="px-3 py-2 text-[13px] text-gray-500 italic">Loading pay heads...</div>
                        ...
                        filteredItems.map((item, idx) => (
                            ...
                            >
                                ...
                                <span className="italic font-normal text-[11px] pr-1">{item.category}</span>
                            </div>
                        ))
                    ) : (
...
export default TallyPayHeadSelectModal;
