...
const TallyAttendanceTypeSelectModal: React.FC<Props> = ({ companyId, onClose, onSelect }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedIndex, setSelectedIndex] = useState(0);
    const searchInputRef = useRef<HTMLInputElement>(null);

    // For now, these are based on the AttendanceType enum in Prisma
    const allItems = [
        { label: 'Present', type: 'item', parent: 'Primary', unit: 'Days' },
        { label: 'Absent', type: 'item', parent: 'Primary', unit: 'Days' },
        { label: 'Leave with Pay', type: 'item', parent: 'Primary', unit: 'Days' },
        { label: 'Leave without Pay', type: 'item', parent: 'Primary', unit: 'Days' }
    ];
        ...
        <div className="absolute inset-0 z-[1000] flex items-start justify-center pt-[55px] pointer-events-none">
            <div className="w-[400px] shadow-[0_15px_60px_rgba(0,0,0,0.4)] flex flex-col pointer-events-auto bg-white border border-[#2a5585]">
                ...
                <div className="bg-[#2a5585] text-white px-2 py-0.5 flex justify-between items-start text-[12px] font-bold min-h-[36px]">
                    <span className="pl-1 pt-0.5">List of Attendance/Production Types</span>
                            ...
                            >
                                <div className="flex-1 flex justify-between">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className="italic font-normal text-[11px] flex items-center gap-1">♦ {item.parent}</span>
                                        <span className="italic font-normal text-[11px] min-w-[30px] text-right">{item.unit}</span>
                                    </div>
                                </div>
                            </div>
                        ))
...
export default TallyAttendanceTypeSelectModal;
